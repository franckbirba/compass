...
exports.CRUD.prototype.put = function(collection,query, callback){
	var tmp = query.body;
	tmp._id = query.body._id ? new ObjectId(query.body._id) : new ObjectId();
    if(tmp._id) delete tmp._id;

	this.db.collection(collection).update({_id:query.body._id},{$set:tmp},{upsert:false},function(err, items){
...
exports.CRUD.prototype.delete = function(collection,query, callback){
	this.db.collection(collection).remove({_id: new ObjectId(query.body._id)},{justOne: true},function(err, items){
...
exports.CRUD.prototype.post = function(collection,query, callback){
    ...
    var id = query.body._id ? new ObjectId(query.body._id) : new ObjectId();
    console.log("POST: "+id);
        if(tmp._id) delete tmp._id;

    this.db.collection(collection).update({_id:id},{$set:tmp},{upsert:true},function(err, items){
        if (items == '1'|| typeof items != 'object'){
            items = [{}];
        }
        callback(err, items);
