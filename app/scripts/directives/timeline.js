(function(){
  ...
  angular
    ...
    .directive('timeline', function ($rootScope) {
      ...
      Controller.$inject = ['$scope', '$http', '$rootScope', 'Auth', 'MODELS', '$route'];
      function Controller($scope, $http, $rootScope, Auth, MODELS, $route){
          ...
          if (newVal && newVal.length) {
           ...
           $scope.onDropComplete=function(index, data, evt){
            ...
            $scope.genTimeline = function() {
                ...
                var dates = [];
                for(var cpt=0;cpt < maxYear - curYear + 1; cpt++) {
                      ...
                      for (var k=0, l=$scope.actions.length; k<l; k++) {
                        ...
                        if ($scope.actions[k].date === tmpTrimIndex) {
                          tmp.push(
                            {
                              count:1,
                              img:$scope.actions[k].img
                            });
                            actionsOnTrim.push($scope.actions[k]);
                        }
                      }
                      trim.push({
                        year: tmpYear,
